...
export const WireframeCanvas: React.FC<WireframeCanvasProps> = ({
  ...
  const getViewportWidth = () => {
    switch (viewport) {
      case 'mobile': return 'max-w-[400px] border-x';
      case 'tablet': return 'max-w-[768px] border-x';
      case 'desktop':
  ...
  const handleNextNote = () => setActiveTestimonialIdx((prev) => (prev + 1) % testimonials.length);
  const handlePrevNote = () => setActiveTestimonialIdx((prev) => (prev - 1 + testimonials.length) % testimonials.length);

  // Wireframe placeholder with cross-lines
  const WireframePlaceholder: React.FC<{ label: string; heightClass?: string; ratioText?: string }> = ({
    ...
    <div className={`relative w-full ${heightClass} ${
      ...
      <div className={`z-10 px-3 py-1.5 rounded text-center backdrop-blur-sm border space-y-1 ${
        isGeometric ? 'bg-white/90 border-slate-200' : 'bg-black/60 border-white/5'
      }`}>
          ...
          <div className="w-full flex items-center justify-between py-4 px-6 md:px-12 relative">
            ...
            {viewport !== 'mobile' ? (
              <div className="flex items-center gap-8 font-mono text-[10px] tracking-widest text-[#a8a29e] uppercase font-medium">
                ...
                <a href="#testimonials" className="hover:text-white transition-colors duration-300">Reflections</a>
                <a href="#footer" className="hover:text-white transition-colors duration-300">Enquiry</a>
              </div>
            ) : null}
            ...
            <div className="flex items-center gap-3">
              {viewport === 'mobile' ? (
                <button
                  ...
                  className={`p-1.5 rounded border ${borderColor} text-white`}
                >
            ...
            <AnimatePresence>
              {viewport === 'mobile' && mobileMenuOpen && (
                ...
                >
                  <a href="#portfolio" onClick={() => setMobileMenuOpen(false)} className="py-1.5 hover:text-white">Portfolio</a>
                  <a href="#services" onClick={() => setMobileMenuOpen(false)} className="py-1.5 hover:text-white">Services</a>
                  <a href="#process" onClick={() => setMobileMenuOpen(false)} className="py-1.5 hover:text-white">Process</a>
                  <a href="#testimonials" onClick={() => setMobileMenuOpen(false)} className="py-1.5 hover:text-white">Reflections</a>
          ...
          <div className="py-12 px-6 md:px-12 space-y-8 relative">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2 text-[10px] font-mono tracking-widest uppercase">
                {(['all', 'private', 'brand', 'corporate'] as const).map((f) => (
                  <button
                    key={f}
                    onClick={(e) => { e.stopPropagation(); setActivePortfolioFilter(f); }}
        ...
        return (
          <div className="py-12 px-6 md:px-12 space-y-8 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4 relative">
              ...
              {[
                { num: '01', phase: 'PHASE_ONE', title: 'Vision Discovery', desc: 'Deep qualitative analysis of our premium clients design values, mood boarding, and atmospheric goals.', metric: '14 Business Days', active: true },
                { num: '02', phase: 'PHASE_TWO', title: 'Spatial Blueprinting', desc: 'Constructing responsive CAD physical space arrangements, camera view templates, and acoustic vector matrices.', metric: 'CAD Blueprint Output', active: false },
                { num: '03', phase: 'PHASE_THREE', title: 'Atelier Fabrication', desc: 'Discreet sourcing, technical fabrication rehearsal of kinetic lighting vectors, and material curation.', metric: 'Material Checklists', active: false },
                { num: '04', phase: 'PHASE_FOUR', title: 'Live Orchestration', desc: 'Atmospheric deployment and real-time live performance oversight, ensuring flawless runtime flow under strict constraints.', metric: 'Real-time Live Log', active: false },
              ].map((step) => (
                <div key={step.num} className="relative group p-4 border border-stone-800/40 rounded bg-stone-950/20">
                  <div className="flex items-center gap-3 mb-4 z-10 relative">
                    <span className={`font-mono text-xs w-9 h-9 rounded-full flex items-center justify-center font-bold border-2 ${
                      step.active
                        ? isChampagne
                          ? 'border-[#cba87c]/50 bg-stone-900 text-[#cba87c]'
                          : 'border-blue-500 bg-slate-900 text-blue-400'
                        : `border-stone-700 text-stone-500 bg-stone-900`
                    }`}>
                      {step.num}
                    </span>
                    <span className="font-mono text-[9px] text-stone-500 tracking-wider">{step.phase}</span>
                  </div>
                  <h3 className="font-sans text-sm font-semibold text-white tracking-tight">{step.title}</h3>
                  <p className="text-xs text-stone-400 mt-2 font-sans leading-relaxed">{step.desc}</p>
                  <div className="mt-3 text-[10px] text-stone-500 font-mono italic">Metrics: {step.metric}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="py-16 px-6 md:px-12 relative flex flex-col items-center">
            ...
            <div className="w-full max-w-3xl text-center space-y-6 relative py-4">
              ...
              <div className="flex justify-center items-center gap-6 pt-4">
                <button
                  onClick={(e) => { e.stopPropagation(); handlePrevNote(); }}
                  className={`p-1.5 rounded-full border ${borderColor} hover:bg-stone-900 transition-all text-white`}
                >
                    ...
                    <span
                      ...
                      className={`w-1.5 h-1.5 rounded-full transition-all ${
                        activeTestimonialIdx === i
                          ? `${highlightBg} ring-1 ${indicatorBorder}`
                          : 'bg-stone-700'
                      }`}
                    />
                  ))}
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleNextNote(); }}
                  className={`p-1.5 rounded-full border ${borderColor} hover:bg-stone-900 transition-all text-white`}
                >
            ...
            <div className="md:col-span-5 space-y-8">
              ...
              <div className="space-y-2 text-[11px] font-mono text-stone-500">
                ...
                <div className="flex justify-between border-b border-stone-900 pb-1.5">
                  ...
                  <span className="text-white cursor-pointer hover:text-[#cba87c]">inquiries@echelon.aura</span>
                </div>
                <div className="flex justify-between border-b border-stone-900 pb-1.5">
              ...
              <div className="pt-4 flex flex-wrap gap-4 text-[10px] font-mono text-stone-600">
            ...
            <div className={`md:col-span-7 p-6 border ${borderColor} rounded-xl ${cardBg} space-y-6`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input type="text" disabled placeholder="e.g. Nolan Sterling"
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" disabled placeholder="e.g. sterling@ateliers.lux"
                    className={`w-full px-3 py-2 text-xs text-stone-300 font-semibold bg-black/40 border ${borderColor} rounded focus:outline-none`} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select disabled className={`w-full px-3 py-2 text-xs text-stone-400 bg-black/40 border ${borderColor} rounded focus:outline-none`}>
              ...
              <div className="space-y-2">
                ...
                <textarea disabled rows={2} placeholder="Tell us about the emotional blueprint, lighting priorities, and target attendees..."
                  className={`w-full px-3 py-2 text-xs text-stone-300 bg-black/40 border ${borderColor} rounded focus:outline-none resize-none`} />
              </div>
              <button type="button" className={`w-full py-2.5 rounded font-mono text-xs uppercase tracking-widest font-bold flex items-center justify-center gap-2 transition-all cursor-not-allowed ${
                isChampagne
  ...
  return (
    <div className={`w-full flex justify-center py-8 px-4 ${containerBg} relative overflow-hidden transition-colors duration-300`}>
      ...
      <div className={`w-full transition-all duration-300 relative shadow-2xl overflow-hidden rounded-md border ${borderColor} ${getViewportWidth()} ${
        ...
        <div className={`p-2 border-b flex justify-between items-center select-none ${borderColor} ${
          isGeometric ? 'bg-slate-100 text-slate-800' : isChampagne ? 'bg-[#1b1b1a]' : 'bg-[#111520]'
        }`}>
            ...
            return (
              <div
                ...
                className={`group relative transition-all duration-300 ${
                  interactiveInspecting ? 'cursor-pointer' : ''
                } ${
                  isInspected
                    ? isGeometric
                ...
                {showLayoutRationale && (
                  <div className="absolute top-2 left-2 z-40 bg-stone-900/90 text-stone-300 border border-stone-700/60 font-sans text-[10px] px-2 py-1 rounded max-w-xs shadow-lg backdrop-blur flex items-start gap-1.5 pointer-events-none select-none">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[9px] leading-tight text-stone-400">{section.rationale.substring(0, 75)}...</p>
                    </div>
                  </div>
                )}
